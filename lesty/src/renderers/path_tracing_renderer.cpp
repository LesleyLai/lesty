...
auto PathTracingRenderer::render_impl(const Scene& scene) -> Image
{

  const auto width = width_;
  const auto height = height_;
  const auto sample_per_pixel = sample_per_pixel_;
  ...
  for (size_t y = 0; y < height_; y += tile_size) {
    for (size_t x = 0; x < width_; x += tile_size) {
      results.push_back(std::async(
          std::launch::async, [this, &progress_tick, tile_count, x, y,
                               sample_per_pixel, width, height, &scene] {
            ...
            for (size_t j = 0; j < tile.height(); ++j) {
              for (size_t i = 0; i < tile.width(); ++i) {
                ...
                for (size_t sample = 0; sample < sample_per_pixel; ++sample) {
                  ...
                  const auto r = camera_.get_ray(Camera_sample{{u, v}});
                  c += trace(scene, r);
                }
